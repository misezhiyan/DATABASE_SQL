...
public class SaveAll {
	...
	private static List<String> getList() {
		...
		paramList.add("tree_solution_0");
		paramList.add("part_info_0");
		paramList.add("fault_trail");
		paramList.add("material_info0");
		paramList.add("fault_code");
		paramList.add("code_solution");
		paramList.add("project_id");
		paramList.add("dealer_id");
		paramList.add("customertype");
		paramList.add("product_model_in");
		paramList.add("product_model_out");
		paramList.add("fault_machine");
		paramList.add("serial_no");
		paramList.add("SERIAL_PRODUCT_ID");
		paramList.add("INNERSERIAL_PRODUCT_ID");
		paramList.add("modu_id");
		paramList.add("model_in_name");
		paramList.add("model_out_name");
		paramList.add("producing_area");
		paramList.add("producing_type");
		paramList.add("product_date");
		paramList.add("project_name");
		paramList.add("usertype");
		paramList.add("serial_no_in");
		paramList.add("dealer_name");
		paramList.add("modu_name");
		paramList.add("installer_id");
		paramList.add("installer_name");
		paramList.add("zx_type");
		paramList.add("zx_type2");
		paramList.add("zx_type3");
		paramList.add("zx_type4 ");
		paramList.add("i_begin_date");
		paramList.add("i_debugging_date");
		paramList.add("local_telphone");
		paramList.add("local_worker_id");
		paramList.add("multiple_calls");
		paramList.add("if_risk");
		return paramList;
	}
	...
	private static void intoList() {
		...
		while (!sn.hasNextInt()) {
			...
			for (String param : params)
				if (!isEmpyt(param))
					System.out.println("paramList.add(\"" + param + "\");");
		}
	}
	...
	private static Map<String, String> getMap() {
		...
		params.put("54", "IF_ZX_TURN");
		params.put("55", "SERVICE_TYPE_TROPESIS");
		params.put("56", "SERVICE_MODE_TROPESIS");
		params.put("57", "if_zj_unreport");
		params.put("58", "REQUIRE_CALLBACK_DATE");
		params.put("59", "project_id");
		params.put("60", "dealer_id");
		params.put("61", "customertype");
		params.put("62", "project_status");
	...
	private static void intoMap() {
		...
		while (!sn.hasNextInt()) {
			...
			String num = line.split("array\\[i\\]\\[")[1].split("\\] = ")[0];
			...
			// if (line.contains("\"+i);")) {
			// String[] commentArr = comment.split("\"\\+i\\);");

...
public class CallBackFail {
	...
	private static void getFromList(List<String> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			System.out.println(list.get(i) + " : " + split[i]);
		}
	}
	...
	private static void getFromParams(Map<String, String> map, List<String> list) {
		...
		while (!sn.hasNextInt()) {
			...
			if (isEmpyt(line))
				continue;
			if (line.startsWith("arr[i][") || line.startsWith("array[i][")) {

				line = line.replace("array", "").replace("arr", "").replace("[i][", "").replace("]", "").trim();
